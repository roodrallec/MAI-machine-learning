...
def friedman_test(accuracies_array, lab, alpha=0.1):
    ...
    if p_value <= alpha:
        ...
        p_values_df = pd.DataFrame(np.round(p_values, 3).transpose())
        p_values_df.columns = lab[:-1]
        p_values_df.index = lab

    return accept, p_value, mean_ranks, p_values_df
...
def w3plot(results, part=1, engine="seaborn", filename=None):
    ...
    if engine == "seaborn":
        ...
        if part == 1:
            ...
            ax = sns.boxplot(x="gamma", y="accuracy", hue="C",
                             data=results, palette="Set1")
            ax.set_title("Parameters Optimization")
            ax.set_ylabel("Accuracy")
